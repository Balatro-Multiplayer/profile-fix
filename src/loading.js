...
function fixCorruption(data){
  return data.replace(/to_big\(\{(\d+\.?\d*)[^}]*\},[^)]*\)/g, (match, number) => {
    return number;
  });
}

export { decompress, compress, rawToJSON, JSONToRaw, processFile, processJSON, FixJSONArrays, FixLuaArrays, fixCorruption};